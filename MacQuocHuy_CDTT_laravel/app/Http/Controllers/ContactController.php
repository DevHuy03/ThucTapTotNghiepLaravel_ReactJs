...
class ContactController extends Controller
{
    public function index($status, $page = 1)
    {
        $contacts = Contact::where('status', '!=', 0)
            ...
            ->select('id', 'name', 'status', 'email', 'title','phone')
            ->get();
        $total = Contact::count();
        $result = [
            'status' => true,
    ...
    public function store(Request $request)
    {
        ...
        if($contact->save())
        {
            ...
            Mail::send('contact', $data, function ($message) use ($data) {
                ...
                $message->to('FashionFlatShop@gmail.com', 'Fashion Flat Shop')
                        ->subject($data['title']);
            });
    ...
    public function trash($status, $page = 1)
    {
        $contact = Contact::where('status', '==', 0)
            ->orderBy('created_at', 'desc')
            ->select('id', 'name', 'user_id', 'status', 'email','content', 'title','phone')
            ->get();
        $total = Contact::count();
        $result = [
            'status' => true,
    ...
    public function status($id)
    {
        ...
        if ($contact->save()) {
            $result = [
                'status' => true,
                'contact' => $contact,
                'message' => 'Đổi trạng thái thành công thành công',
